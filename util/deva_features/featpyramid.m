...
for i = 1:length(pyra.feat)
    ...
    padchannel = size(pyra.feat{i},3) - (size(im,3)-3);
    pyra.feat{i}(1:pady+1, :, padchannel) = 1;
    pyra.feat{i}(end-pady:end, :, padchannel) = 1;
    pyra.feat{i}(:, 1:padx+1, padchannel) = 1;
    pyra.feat{i}(:, end-padx:end, padchannel) = 1;
end
